...
public class Network extends Thread {
	...
	private static Semaphore outSemFull, inSemFull, outSemEmpty, inSemEmpty;
	/** 
	...
	{ 
		...
		outSemFull = new Semaphore(maxNbPackets);
		inSemFull = new Semaphore(maxNbPackets);
		outSemEmpty = new Semaphore(0);
		inSemEmpty = new Semaphore(0);
...
//		inSemFull.release();
	...
	{	
		...
		while (true)
		{
			if(Network.getClientConnectionStatus().equals("disconnected") && Network.getServerConnectionStatus().equals("disconnected"))
				break;
			else
				Thread.yield();
		}
		System.out.println("\n Terminating network thread - Client " + Network.getClientConnectionStatus() + " Server " + Network.getServerConnectionStatus());
	}
}
